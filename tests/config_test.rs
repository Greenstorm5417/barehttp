...
fn test_config_with_timeout() -> Result<(), Error> {
  ...
  let response = client.get("http://httpbin.org/delay/1").call()?;
...
fn test_config_custom_user_agent() -> Result<(), Error> {
  ...
  let mut client = HttpClient::with_config(config)?;
  let response = client.get("http://httpbin.org/user-agent").call()?;
...
fn test_config_max_redirects() -> Result<(), Error> {
  ...
  let response = client.get("http://httpbin.org/redirect/2").call()?;
  assert!(response.is_success());
...
fn test_multiple_requests_same_client() -> Result<(), Error> {
  ...
  // First request
  let response1 = client.get("http://httpbin.org/get").call()?;
  assert!(response1.is_success());

  // Second request with different path
  let response2 = client.get("http://httpbin.org/user-agent").call()?;
